...
function getMinorAJAX() {
    ajax(GET, {path: "/minors" }, '#minors').done(function(jsonMINOR) {
        ...
        $.each(minorsJSON, function(){
            $('#minors').append('<a href="#linkModal" rel="modal:open" id="minorsAnchor">' +'<div class="eacMinor" dataMinorName="'+this.name+'">'+'<p class="degreeName">' + this.title + '</p>'+'</div>' + '</a>');
...
function MinorsBackModal(BackModal, dataBack){
    ...
    var dataJ = getAttributesByName(BackModal, 'name', dataBack);
    ...
    if( $('.modallength').length > 0 ){
        ...
        // Backmodal 
        var back = '<h2>' + dataJ.title + '</h2>' +
                            '<p class="minor-description">' + dataJ.description + "</p>" +
                            '<h3> Courses: </h3>' + 
                            '<ul class="minor-courses">';

        // courses array
        $.each(dataJ.courses, function(index , elem){
            back = back + '<li>' + elem + '</li>'; 
        });

        
        if(dataJ.note){
            back = back + '</ul><p class="minor-note">*'+ dataJ.note +'</p>';
        }
        else{
            back = back + '</ul>';
        }

        $('#linkModal').append(back); // append back modal to the MAIN modal
    }
    else{
        
        var back = '<div id="linkModal" class="modal modallength">' +
                            '<h2>' + dataJ.title + '</h2>' +
                            '<p class="minor-description">' + dataJ.description + "</p>" +
                            '<h3> Courses: </h3>' + 
                            '<ul class="minor-courses">';
        ...
        if(dataJ.note){
            back += '</ul><p class="minor-note">*'+ dataJ.note +'</p></div>';
        }
        else{
            back += '</li></div>';
        }

        $('body').append(back); // append back modal to the dom
    }
}
